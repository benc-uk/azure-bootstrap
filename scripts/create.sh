...
az pipelines create --name 'Deploy Azure Core' --yml-path /pipelines/deploy-core.yml --repository $REPO_SOURCE --branch master --service-connection $GITHUB_CONN_ID --project $ADO_PROJECT --organization $ADO_ORG -o table
